...
void print_usage() {
    fprintf(stderr, "Usage: eval-pct train_file test_file\n");
    return;
}
...
int main(int argc, char **argv) {
  ...
  param.model_type = toybox::BINARY;
  param.T = 10;
  param.verbose = false;

  while ((ch = getopt(argc, argv, "vt:m:")) != -1) {
    switch (ch) {
      case 'm':
        param.model_type = static_cast<int>(strtol(optarg, NULL, 10));
        break;
      case 'v':
        param.verbose = true;
        break;
  ...
  toybox::Perceptron *pct = new toybox::Perceptron();

  std::vector<toybox::SVector> x_vec;
  std::vector<int> y_vec;

  read_data_file(train_f, &x_vec, &y_vec);
  pct->Train(x_vec, y_vec, param);

  /*
  pct->Save("test.model");
  delete pct; pct = NULL;
  pct = new toybox::Perceptron();
  pct->Load("test.model");
  */
  ...
  int *labels = new int[pct->lnum()];
  pct->GetLabels(labels);

  read_data_file(test_f, &x_vec, &y_vec);
  int num_correct = 0;
  for (size_t i = 0; i < x_vec.size(); ++i) {
    int max_label = pct->Predict(x_vec[i], NULL);
