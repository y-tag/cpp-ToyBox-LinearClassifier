...
void print_usage() {
    fprintf(stderr,
            "Usage: eval-svmsgd train_file test_file\n"
            "options:\n"
...
int main(int argc, char **argv) {
  ...
  param.model_type = toybox::SGD2_BIN_L1;
  param.T = 10;
  param.t0 = 1e6;
  param.lambda = 1e-2;
  param.skip = 32;
  ...
  while ((ch = getopt(argc, argv, "m:z:t:l:s:v")) != -1) {
    switch (ch) {
      case 'm':
        param.model_type = static_cast<toybox::ModelType>(strtol(optarg, NULL, 10));
        break;
      case 'z':
        param.t0 = static_cast<int>(strtol(optarg, NULL, 10));
        break;
      case 't':
        param.T = static_cast<int>(strtol(optarg, NULL, 10));
        break;
      case 'l':
        param.lambda = strtod(optarg, NULL);
        break;
      case 's':
        param.skip = static_cast<int>(strtol(optarg, NULL, 10));
        break;
      case 'v':
        param.verbose = true;
        break;
  ...
  toybox::SVMSGD *classifier = new toybox::SVMSGD();
